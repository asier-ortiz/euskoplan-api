    ...
    public function run($language)
    {
        ...
        foreach ($jsonData as $obj) {
            ...
            if (isset($obj->fechaActualizacion)) {
                ...
                // Creación de los espacios naturales
                $natural = Natural::updateOrCreate([
                    ...
                    'subTipoRecursoEspacioNatural' => $obj->datosEspaciosNaturales->subTipoRecurso ?? null,
                    'nombreSubTipoRecursoEspacioNatural' => $obj->datosEspaciosNaturales->nombreSubTipoRecurso ?? null,
                    'fauna' => $obj->datosEspaciosNaturales->fauna ?? null,
                    'flora' => $obj->datosEspaciosNaturales->flora ?? null,

                    // Datos playas, pantanos y ríos
                    'subTipoRecursoPlayasPantanosRios' => $obj->datosPlayasPantanosRios->subTipoRecurso ?? null,
                    'nombreSubTipoRecursoPlayasPantanosRios' => $obj->datosPlayasPantanosRios->nombreSubTipoRecurso ?? null,
                    'horario' => $obj->datosPlayasPantanosRios->horario ?? null,
                    'actividades' => $obj->datosPlayasPantanosRios->actividades ?? null

                ]);

                // Creación de los servicios y asociación al espacio natural
                    ...
                    foreach ($obj->servicios->servicio as $service) {
                        ...
                        if ($filteredArr) {
                            foreach ($filteredArr as $filteredService) {
                                $natural->services()->syncWithoutDetaching($filteredService->id);
                            }
                        }
                    }
                }

                // Creación de las imágenes y asociación al espacio natural
                        ...
                        foreach ($obj->galeria as $image) {
                            if (isset($image->src)) {
                                Image::updateOrCreate([
                                    ...
                                    'imageable_id' => $natural->id,
                                    'imageable_type' => 'natural'
                                ]);
                            }
                        }
                    } else {
