...
class MuseumSeeder extends Seeder
{
    ...
    public function run($language)
    {
        ...
        $path = database_path() . '/data/' . $language . '-museums.json';
        ...
        foreach ($jsonData as $obj) {
            ...
            if (isset($obj->fechaActualizacion)) {
                ...
                $museum = Museum::updateOrCreate([
                    ...
                    'subTipoRecurso' => $obj->datosArteCultura->subTipoRecurso ?? null,
                    'nombreSubTipoRecurso' => $obj->datosArteCultura->nombreSubTipoRecurso ?? null,
                    'tematica' => $obj->datosArteCultura->tematica ?? null,
                    'nombreTematica' => $obj->datosArteCultura->nombreTematica ?? null,
                    'capacidad' => $obj->datosArteCultura->capacidad ?? null,
                    'horario' => $obj->datosArteCultura->horario ?? null

                ]);

                // Creación de los servicios y asociación al museo
                    ...
                    foreach ($obj->servicios->servicio as $service) {
                        ...
                        if ($filteredArr) {
                            foreach ($filteredArr as $filteredService) {
                                $museum->services()->syncWithoutDetaching($filteredService->id);
                            }
                        }
                    }
                }

                // Creación de las imágenes y asociación al museo
                        ...
                        foreach ($obj->galeria as $image) {
                            if (isset($image->src)) {
                                Image::updateOrCreate([
                                    ...
                                    'imageable_id' => $museum->id,
                                    'imageable_type' => 'museum'
                                ]);
                            }
                        }
                    } else {
